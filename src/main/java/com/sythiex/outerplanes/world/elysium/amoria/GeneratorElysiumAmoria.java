...
public class GeneratorElysiumAmoria implements IWorldGenerator
{
	...
	{
		if(world.provider.dimensionId == OuterPlanes.dimIDAmoria)
		{
	...
	private void generateWaterPortal(World world, Random rand, int chunkX, int chunkZ)
	{
		...
		if(rand.nextInt(20) == 0)
		{
			...
			// checks surrounding chunks for other portals
			for(int i = -8; i <= 8; i++)
			{
				for(int k = -8; k <= 8; k++)
				{
			...
			if(world.getBlock(x, y, z) == Blocks.water)
			{
				...
				if(!flag)
				{
					int minY = y - (rand.nextInt(14) + 3); // number between 48 and 61
					...
					for(int j = minY - 1; j >= 48; j--)
					{
						for(int k = -3; k <= 3; k++)
						{
							for(int i = -3; i <= 3; i++)
							{
								...
								{
									world.setBlock(x + i, j, z + k, Blocks.stonebrick, 0, 3);
								}
							}
						}
					}
	...
	private int getTopBlockY(World world, int x, int z)
	{
		...
		while((!world.getBlock(x, y, z).getMaterial().blocksMovement()
