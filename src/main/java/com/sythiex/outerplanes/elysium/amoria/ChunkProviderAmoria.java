...
public class ChunkProviderAmoria implements IChunkProvider
{
	/** RNG. */
	private Random rand;
	...
	public ChunkProviderAmoria(World world, long seed, boolean mapFeaturesEnabled)
	{
		this.worldObj = world;
	...
	public void replaceBlocksForBiome(int chunkX, int chunkZ, Block[] ablock, byte[] abyte, BiomeGenBase[] biomesForGeneration)
	{
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this, chunkX, chunkZ, ablock, abyte, biomesForGeneration, this.worldObj);
		MinecraftForge.EVENT_BUS.post(event);
		if(event.getResult() == Result.DENY)
			return;
			
		double d0 = 0.03125D;
		this.stoneNoise = this.noisePer1.func_151599_a(this.stoneNoise, (double) (chunkX * 16), (double) (chunkZ * 16), 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D);
	...
	public void populate(IChunkProvider chunkProvider, int p_73153_2_, int p_73153_3_)
	{
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(chunkProvider, worldObj, rand, p_73153_2_, p_73153_3_, flag));
		...
		if(biomegenbase != BiomeGenBase.desert && biomegenbase != BiomeGenBase.desertHills && !flag && this.rand.nextInt(4) == 0 && TerrainGen.populate(chunkProvider, worldObj, rand, p_73153_2_, p_73153_3_, flag, LAKE))
		{
		...
		boolean doGen = TerrainGen.populate(chunkProvider, worldObj, rand, p_73153_2_, p_73153_3_, flag, DUNGEON);
		...
		if(TerrainGen.populate(chunkProvider, worldObj, rand, p_73153_2_, p_73153_3_, flag, ANIMALS))
		{
		...
		doGen = TerrainGen.populate(chunkProvider, worldObj, rand, p_73153_2_, p_73153_3_, flag, ICE);
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Post(chunkProvider, worldObj, rand, p_73153_2_, p_73153_3_, flag));
	...
	public List getPossibleCreatures(EnumCreatureType creatureType, int x, int y, int z)
	{
		...
		// prevents monsters from spawning
		return creatureType != EnumCreatureType.monster ? biomegenbase.getSpawnableList(creatureType) : null;
	}
